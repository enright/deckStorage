...
module.exports.start = function (port) {
	...
	app.post('/deck', function (req, res) {
		...
		if (deck) {
			dynamo.putItem({ TableName: 'dealItUp',
				Item: { id: { S: aUuid }, deck: { S: JSON.stringify(deck) }}}, function (err) {
				if (err) {
					res.json(400, { error: 'database error', dbError: err});
				} else {
			...
			res.json(400, { error: 'no deck specified' });
		}
	...
	app.put('/deck', function (req, res) {
		...
		if (id && deck) {
			dynamo.putItem({ TableName: 'dealItUp',
				Item: { id: { S: id }, deck: { S: JSON.stringify(deck) }}},
				function (err) {
					if (err) {
						res.json(400, { error: 'database error' , dbError: err });
					} else {
			...
			res.json(400, { error: 'deck and id must be specified', id: id });
		}
	});
