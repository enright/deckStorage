...
module.exports.start = function (port) {
	...
	app.post('/deck', function (req, res) {
		...
		if (deck && Array.isArray(deck) &&
			drawn && Array.isArray(drawn) &&
			discarded && Array.isArray(discarded) &&
			removed && Array.isArray(removed) ) {
			...
			res.json(400, { error: 'deck, drawn, discarded and removed must be specified as arrays' });
		}
	...
	app.put('/deck', function (req, res) {
		...
		if (id &&
			...
			res.json(400, { error: 'id, deck, drawn, discarded and removed must be specified as arrays', id: id });
		}
	});
